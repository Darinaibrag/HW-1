...
- "src-layout": a directory representing a Python taskAPI that contains a "src"
  folder. Everything under the "src" folder is meant to be included in the
  distribution when packaging the taskAPI. Example::
...
- "flat-layout": a Python taskAPI that does not use "src-layout" but instead
  have a directory under the taskAPI root for each package::
...
- "single-module": a taskAPI that contains a single Python script direct under
  the taskAPI root (no directory used)::
...
class ConfigDiscovery:
    ...
    def __call__(self, force=False, name=True, ignore_ext_modules=False):
        ...
        To repeatedly invoke automatic discovery (e.g. when the taskAPI
    ...
    def _analyse_flat_layout(self) -> bool:
        """Try to find all packages and modules under the taskAPI root.
