...
def validate_project_dynamic(pyproject: T) -> T:
    project_table = pyproject.get("taskAPI", {})
    dynamic = project_table.get("dynamic", [])

    for field in dynamic:
        if field in project_table:
            msg = f"You cannot provide a value for `taskAPI.{field}` and "
            msg += "list it under `taskAPI.dynamic` at the same time"
            name = f"data.taskAPI.{field}"
